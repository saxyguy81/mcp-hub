...
class ProxyTestSuite:
    ...
    def test_proxy_commands_available(self):
        ...
        self.log_test(
            ...
            result["success"],
            (
                "Proxy subcommand group registered"
                if result["success"]
                else result["stderr"]
            ),
        )
    ...
    def test_services_setup(self):
        ...
        if not compose_file.exists():
            ...
            if not result["success"]:
                self.log_test(
                    "Services setup", False, "Failed to generate compose file"
                )
                return False
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/health", timeout=10) as resp:
                    if resp.status == 200:
                        ...
                        self.log_test(
                            "Proxy health check", False, f"HTTP {resp.status}"
                        )
                        return False
        except Exception as e:
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        if "result" in data:
                            ...
                            self.log_test(
                                "MCP initialize", False, f"Error: {data.get('error')}"
                            )
                            return False
                    else:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        self.log_test(
                            "Tools list aggregation", False, f"HTTP {resp.status}"
                        )
                        return False
        except Exception as e:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        self.log_test(
                            "Resources list aggregation", False, f"HTTP {resp.status}"
                        )
                        return False
        except Exception as e:
