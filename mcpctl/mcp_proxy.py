...
class MCPProxy:
            ...
            for service_name, config in services.items():
                ...
                for port_mapping in ports:
                    if isinstance(port_mapping, str) and ':' in port_mapping:
                        ...
                        discovered_count += 1
                        self.logger.info(f"Discovered MCP server: {service_name} at {url}")
                        break
            ...
            async with self.session.post(
                ...
                if resp.status == 200:
                    ...
                    if "result" in data:
                        ...
                        self.logger.info(f"Initialized server {server.name} successfully")
                    else:
                        self.logger.warning(f"Server {server.name} initialization returned error: {data.get('error')}")
                        server.healthy = False
                else:
                    self.logger.warning(f"Server {server.name} initialization failed: HTTP {resp.status}")
                    server.healthy = False
    ...
    async def handle_initialize(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Aggregating capabilities from {len(healthy_servers)} healthy servers")
    ...
    async def handle_tools_list(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Aggregated {len(all_tools)} tools from {len(self.servers)} servers")
        ...
        if target_server_name and target_server_name in self.servers:
            ...
            if target_server.healthy:
                try:
                    ...
                    self.logger.error(f"Failed to call tool {tool_name} on {target_server_name}: {e}")
    ...
    async def handle_resources_list(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def handle_resources_read(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if target_server_name and target_server_name in self.servers:
            ...
            if target_server.healthy:
                try:
                    ...
                    self.logger.error(f"Failed to read resource {resource_uri} from {target_server_name}: {e}")
    ...
    async def broadcast_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "error": {
                "code": -32601,
                "message": "Method not found on any server"
            }
        }
...
async def handle_health(request):
    ...
    return web.json_response({
        ...
        "server_list": [name for name, info in status["servers"].items() if info["healthy"]]
    })
...
async def handle_servers(request):
    ...
    for name, server in proxy.servers.items():
        servers.append({
            ...
            "url": server.url,
            "healthy": server.healthy,
            "last_check": datetime.fromtimestamp(server.last_check).isoformat() if server.last_check else None,
            "error_count": server.error_count,
...
async def main():
    ...
    parser.add_argument("--config", type=str, default="docker-compose.yml", help="Docker compose file path")
