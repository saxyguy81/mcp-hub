...
try:
    ...
    from .workspace import MCPWorkspace, WorkspaceManager
except ImportError as e:
...
def get_docker_client():
    ...
    if not DOCKER_AVAILABLE:
        ...
        typer.echo(
            "🔧 Or use Docker CLI fallback (automatic in most commands)", err=True
        )
        raise typer.Exit(1)
...
def init(
    ...
    registry: str = typer.Option(
        "offline", help="Registry driver (ghcr|gitlab|offline)"
    ),
    ...
    config = MCPConfig(
        git_remote=git_remote, registry_driver=registry, secrets_backend=secrets
    )
...
def discover(
    ...
    output: str = typer.Option(
        "services", help="Output directory for service definitions"
    ),
):
...
def generate(
    services_dir: str = typer.Option(
        "services", help="Directory containing service definitions"
    ),
...
def remove(
    ...
    if not typer.confirm(
        f"Remove service '{name}'? This will stop and delete the container."
    ):
        raise typer.Exit(1)
...
def test(
    ...
    try:
        ...
        result = container_engine.run(
            ["compose", "ps", name], capture_output=True, text=True
        )
        ...
        health_result = container_engine.exec_service(
            name, ["curl", "-fsSL", f"http://localhost:8080{endpoint}"]
        )
...
def daemon(
    ...
    restart_on_failure: bool = typer.Option(
        True, help="Auto-restart services on failure"
    ),
):
                    ...
                    if line:
                        try:
                            ...
                            if state != "running" and restart_on_failure:
                                logger.warning(
                                    f"Service {service_name} is {state}, restarting..."
                                )
                                container_engine.compose_up(
...
def regenerate_bridge(
    ...
    restart_service: bool = typer.Option(
        True, help="Restart mcp-openapi container after regeneration"
    ),
):
    ...
    if not config_path.exists():
        ...
        typer.echo(
            "Run setup wizard first or specify config file with --config-file", err=True
        )
        raise typer.Exit(1)
    ...
    bridge_config = {
        ...
        "model": (
            "gpt-3.5-turbo"
            if llm_config.get("llmBackend") == "openai"
    ...
    if restart_service:
        try:
            ...
            typer.echo(
                "You may need to restart it manually with: docker compose restart mcp-openapi"
            )
...
def lock_images(
    compose_file: str = typer.Option(
        "docker-compose.yml", help="Docker compose file to scan for images"
    ),
    output_file: str = typer.Option("images.lock.json", help="Output lock file"),
    additional_images: List[str] = typer.Option(
        [], help="Additional images to lock (beyond compose file)"
    ),
):
...
def pull_images(
    lock_file: str = typer.Option(
        "images.lock.json", help="Lock file with image digests"
    ),
    verify_digests: bool = typer.Option(
        True, help="Verify pulled images match expected digests"
    ),
):
...
workspace_app = typer.Typer(
    name="workspace", help="Manage MCP configuration workspaces"
)
...
def workspace_create(
    ...
    encrypt_secrets: bool = typer.Option(
        True, help="Encrypt secrets in git repository"
    ),
    use_lastpass: bool = typer.Option(
        True, help="Use LastPass for encryption key storage"
    ),
):
        ...
        if encrypt_secrets and workspace.secrets:
            ...
            for secret_name, description in workspace.secrets.items():
                if secret_name in os.environ:
                    ...
                    value = typer.prompt(
                        f"Enter value for {secret_name} ({description})",
                        hide_input=True,
                    )
                    actual_secrets[secret_name] = value

            # Encrypt the secrets
            encrypted_data = encryption_manager.encrypt_secrets(
                actual_secrets, name, use_lastpass
            )
...
def workspace_export(
    ...
    try:
        ...
        if format == "git":
            typer.echo(
                f"🔧 To share: cd {output_path}/{name} && git init && git add . && git commit -m 'Initial workspace'"
            )
        elif format == "bundle":
    ...
    if source.startswith(("http://", "https://", "git@")):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if (
                    workspace
                    and isinstance(workspace.secrets, dict)
                    and workspace.secrets.get("encrypted")
                ):
                    typer.echo(
                        f"🔐 Workspace '{workspace_name}' uses encrypted secrets"
                    )
                    ...
                    if encryption_manager.import_key_setup(workspace_name):
                        typer.echo(
                            f"✅ Imported encrypted workspace '{workspace_name}' from git repository"
                        )
                    else:
                        typer.echo(
                            f"⚠️  Imported workspace '{workspace_name}' but secrets remain encrypted"
                        )
                        typer.echo(
                            "💡 Use 'mcpctl workspace decrypt {workspace_name}' to set up decryption later"
                        )
                else:
                    typer.echo(
                        f"✅ Imported workspace '{workspace_name}' from git repository"
                    )
...
def workspace_info(
    ...
    if not name:
        ...
        if not name:
            typer.echo(
                "❌ No active workspace. Specify a name or activate one.", err=True
            )
            raise typer.Exit(1)
...
def workspace_sync(
    ...
    typer.echo(
        "💡 For now, use 'workspace export --format git' and manual git operations"
    )
...
def llm_test(
    backend: str = typer.Option(
        "all", help="Backend to test: claude, openai, custom, or all"
    ),
    ...
    if backend == "all":
        ...
        if not config.get("openai_api_key"):
            ...
            print(
                "💡 Provide with --api-key or set OPENAI_API_KEY environment variable"
            )
            return
        results["openai"] = tester.test_openai_api(config["openai_api_key"])
                ...
                if result["success"]:
                    if backend_name == "openai" and config.get("openai_api_key"):
                        ...
                        if config.get("custom_llm_api_key"):
                            existing_config["custom_llm_api_key"] = config[
                                "custom_llm_api_key"
                            ]
                        if config.get("custom_llm_model"):
                            existing_config["custom_llm_model"] = config[
                                "custom_llm_model"
                            ]
...
def llm_setup(interactive: bool = typer.Option(True, help="Interactive setup wizard")):
    ...
    if interactive:
        ...
        if setup_openai:
            ...
            if api_key:
                ...
                if result["success"]:
                    ...
                    save_anyway = (
                        input("Save configuration anyway? (y/N): ")
                        .lower()
                        .startswith("y")
                    )
                    if save_anyway:
        ...
        # Custom LLM setup
        setup_custom = (
            input("Configure custom LLM endpoint? (y/N): ").lower().startswith("y")
        )
        if setup_custom:
            url = input(
                "Enter custom LLM URL (e.g., https://api.your-provider.com): "
            ).strip()
