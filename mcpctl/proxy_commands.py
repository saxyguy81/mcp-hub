...
try:
    from .cli import app as main_app, load_config
except ImportError:
...
def start_proxy(
    ...
    auto_start_services: bool = typer.Option(True, help="Auto-start MCP services if not running")
):
...
def stop_proxy():
    ...
    if not pid:
        ...
        if not status["running"]:
            ...
            typer.echo("⚠️  Proxy running but PID file missing. Trying to stop via HTTP...")
...
def proxy_status():
    ...
    if status["healthy"] and status["data"]:
        data = status["data"]
        typer.echo(f"📊 Backend Servers: {data.get('healthy_servers', 0)}/{data.get('servers', 0)} healthy")
...
def proxy_logs(
    ...
    try:
        if follow:
            ...
            subprocess.run([
                "tail", "-f", "-n", str(lines), str(PROXY_LOG_FILE)
            ])
        else:
            # Show last N lines
