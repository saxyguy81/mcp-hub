...
class EncryptionManager:
    ...
    def prompt_for_key(self, workspace_name: str = "") -> str:
        ...
        prompt = f"🔐 Enter encryption key for workspace '{workspace_name}': " if workspace_name else "🔐 Enter encryption key: "
    ...
    def get_encryption_key(self, workspace_name: str, use_lastpass: bool = True) -> bytes:
        ...
        if not key_password:
            ...
            if use_lastpass:
                store = input("💾 Store this key in LastPass for future use? (y/N): ").lower().startswith('y')
                if store:
    ...
    def encrypt_data(self, data: Dict[str, Any], workspace_name: str, use_lastpass: bool = True) -> str:
    ...
    def decrypt_data(self, encrypted_string: str, workspace_name: str, use_lastpass: bool = True) -> Dict[str, Any]:
    ...
    def encrypt_secrets(self, secrets: Dict[str, str], workspace_name: str, use_lastpass: bool = True) -> str:
        """Encrypt secrets dictionary"""
        return self.encrypt_data(secrets, workspace_name, use_lastpass)
    
    def decrypt_secrets(self, encrypted_secrets: str, workspace_name: str, use_lastpass: bool = True) -> Dict[str, str]:
    ...
    def import_key_setup(self, workspace_name: str, key_hint: str = "") -> bool:
        ...
        if "LastPass" in key_hint:
            ...
            use_lastpass = input("🔑 Try to retrieve key from LastPass? (Y/n): ").lower() != 'n'
        else:
            print("💡 This workspace uses a manually entered encryption key")
            use_lastpass = input("🔑 Check LastPass for encryption key? (y/N): ").lower().startswith('y')
...
class SecureWorkspaceManager:
    ...
    def create_encrypted_workspace(self, name: str, description: str, secrets: Dict[str, str], 
                                 services: Dict[str, Any], use_lastpass: bool = True) -> None:
        """Create workspace with encrypted secrets"""
        
        # Encrypt secrets
        encrypted_secrets = self.encryption_manager.encrypt_secrets(secrets, name, use_lastpass)
    ...
    def load_and_decrypt_workspace(self, name: str, use_lastpass: bool = True) -> Optional[Dict[str, str]]:
        ...
        if isinstance(workspace.secrets, dict) and workspace.secrets.get("encrypted"):
            try:
                ...
                decrypted_secrets = self.encryption_manager.decrypt_secrets(encrypted_data, name, use_lastpass)
