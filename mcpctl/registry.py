...
class RegistryManager:
    ...
    def _save_images_tarball_cli(self, tag: str = "latest") -> Path:
        ...
        typer.echo(f"Saving {len(images_to_save)} MCP images to tarball using Docker CLI")
                ...
                if image.tags:
                    for image_tag in image.tags:
                        if registry in image_tag:
                            repository, tag_part = image_tag.rsplit(':', 1) if ':' in image_tag else (image_tag, 'latest')
        ...
        try:
            ...
            if web_dockerfile.exists():
                ...
                image, logs = self.client.images.build(
                    path=".", dockerfile="web/Dockerfile", 
                    tag=f"{image_name}:{tag}", rm=True, pull=True
                )
                if tag != "latest":
